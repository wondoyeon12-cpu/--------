...
def main():
    ...
    # [NEW] Parse dynamic viewport and mobile flag
    width = int(sys.argv[3]) if len(sys.argv) > 3 else 1280
    height = int(sys.argv[4]) if len(sys.argv) > 4 else 1080
    is_mobile = sys.argv[5].lower() == 'true' if len(sys.argv) > 5 else False
    
    try:
        with open("playwright_debug.log", "a", encoding="utf-8") as f:
            f.write(f"[{time.time()}] Started for URL: {url} | Target: {output_filename} | Mobile: {is_mobile}\n")
